...
export class RoomsRepo {
  ...
  async getAllRooms(limit: number = 50, offset: number = 0): Promise<{ data: Room[]; total: number }> {
    const totalResult = await this.env.DB.prepare(`SELECT COUNT(*) as count FROM rooms`).first<{ count: number }>();
  ...
  async deleteRoom(roomId: string): Promise<boolean> {
    const result = await this.env.DB.prepare(`DELETE FROM rooms WHERE room_id = ?1`)
      .bind(roomId)
      .run();
