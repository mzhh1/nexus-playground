...
app.post('/init', async (c) => {
    ...
    try {
        const state = await logic.initState(body);
        return c.json(state);
    } catch (error: any) {
...
app.post('/legal-actions', async (c) => {
    ...
    try {
        const actions = await logic.getLegalActions(body.state, body.roleId);
...
app.post('/action', async (c) => {
    ...
    try {
        const result = ('validateAndApply' in logic)
            ? await (logic as any).validateAndApply(body.state, body.action)
            : await logic.applyAction(body.state, body.action);
...
app.post('/is-terminal', async (c) => {
    ...
    try {
        const isTerminal = await logic.isTerminal(body.state);
        const winners = isTerminal ? await logic.getWinners(body.state) : null;
...
app.post('/perspective', async (c) => {
    ...
    try {
        const perspective = await logic.toRolePerspective(
            body.state,
            body.roleId,
            body.wholeHistory || [],
            body.diffHistory || []
...
app.post('/current-role', async (c) => {
    const body = await c.req.json<{ state: GameState }>();
    try {
        const roleId = await logic.getCurrentRole(body.state);
