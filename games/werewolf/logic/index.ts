...
// 导入视角生成
import {
  toRolePerspective as toRolePerspectiveImpl,
  generateStatePrompt as generateStatePromptImpl,
} from './perspective.js';
...
export class WerewolfLogic extends BaseGameLogic<WerewolfState> {
  ...
  getMetadata(): GameMetadata {
    return {
      ...
      name: '狼人杀 (Werewolf)',
      description:
        '经典狼人杀：狼人阵营与好人阵营的推理博弈，包含预言家、女巫、猎人、守卫等神职角色以及平民。',
      minPlayers: 6,
      maxPlayers: 12,
      roleIds: PLAYER_ROLE_IDS_BY_COUNT,
      playerCountLabels: PLAYER_COUNT_LABELS,
      ...
      getStatusText: (perspective: RolePerspective) => {
        const current = perspective.current_state as Partial<WerewolfState['currentNightActions']> & {
          ...
          day?: number;
          nightSubPhase?: import('./types.js').NightSubPhase;
          winner?: import('./types.js').Camp | null;
        };

        if (current?.winner) {
          return `游戏结束 - ${current.winner === 'werewolf' ? '狼人阵营' : '好人阵营'}获胜`;
        }
        ...
        if (current?.phase === 'day_discussion') {
          return `第 ${current?.day ?? 1} 天 - 白天讨论`;
        }

        if (current?.phase === 'day_voting') {
          return `第 ${current?.day ?? 1} 天 - 公投放逐`;
        }

        if (current?.phase === 'last_words') {
          return `第 ${current?.day ?? 1} 天 - 遗言阶段`;
        }

        if (current?.phase === 'hunter_shoot') {
          return `第 ${current?.day ?? 1} 天 - 猎人发动技能`;
        }
  ...
  initState(ctx: InitContext): GameState {
    ...
    const alive: Record<string, boolean> = Object.fromEntries(
      ctx.players.map((playerId) => [playerId, true])
    );
  ...
  getLegalActions(state: GameState, roleId: string): ActionSpec {
    ...
    switch (s.phase) {
      ...
      case 'last_words':
        return { actions: [{ action_id: 'last_words', description: '发表遗言', params_schema: { content: { type: 'string', description: '请输入你的遗言' } } }] };
      case 'hunter_shoot':
  ...
  toRolePerspective(
    _state: GameState,
    _roleId: string,
    _wholeHistory: HistoryEvent[],
    _diffHistory: HistoryEvent[]
  ): RolePerspective {
    const state = _state as WerewolfState;
    const roleId = _roleId;
    const wholeHistory = _wholeHistory;
    const diffHistory = _diffHistory;
  ...
  private applyDeaths(
    state: WerewolfState,
    deaths: import('./types.js').DeathRecord[],
    resumePhaseIfHunter: 'day_discussion' | 'night'
  ): void {
    applyDeaths(state, deaths, resumePhaseIfHunter);
  }

  private resumeAfterHunter(state: WerewolfState, shotDeath: import('./types.js').DeathRecord | null): void {
    resumeAfterHunter(
      state,
      shotDeath,
      (s, deaths, resumePhase) => this.applyDeaths(s, deaths, resumePhase),
  ...
  private applyDayDiscussionAction(state: WerewolfState, action: Action): ActionResult {
    const consumeActor = (s: WerewolfState) => consumeCurrentActor(s);
    const result = applyDayDiscussionAction(state, action, consumeActor);
    if (result.success) {
      this.finalizeAction(state);
    }
    return result;
  ...
  private applyDayVotingAction(state: WerewolfState, action: Action): ActionResult {
    const consumeActor = (s: WerewolfState) => consumeCurrentActor(s);
    const result = applyDayVotingAction(state, action, consumeActor);
    if (result.success) {
      this.finalizeAction(state);
    }
    return result;
  ...
  private applyLastWordsAction(state: WerewolfState, action: Action): ActionResult {
    if (action.action_id !== 'last_words') {
      return {
        success: false,
        error: '无效的遗言行动',
    ...
    if (!context) {
      return {
        success: false,
        error: '当前没有待发表遗言的玩家',
        errorCode: 'NO_LAST_WORDS_CONTEXT',
      };
    }
    ...
    if (!pendingDeath) {
      return {
        success: false,
        error: '你不在待发表遗言的名单中',
        errorCode: 'NOT_IN_LAST_WORDS_LIST',
      };
    }

    // 记录遗言
    state.lastWordsHistory.push({
      day: state.day,
    ...
    if (state.pendingRoles.length === 0) {
      this.resumeAfterLastWords(state);
    }
  ...
  private resumeAfterLastWords(state: WerewolfState): void {
    ...
    // 如果没有触发猎人开枪，根据 resumePhase 恢复游戏流程
    if (state.phase !== 'hunter_shoot') {
      if (resumePhase === 'day_discussion') {
        startDayDiscussion(state);
      } else if (resumePhase === 'night') {
    ...
    for (let i = 0; i < deaths.length; i += 1) {
      ...
      if (state.identities[victim] === 'hunter') {
        ...
        if (state.hunterCanShoot && record.cause !== 'poison') {
          ...
          state.hunterShootContext = {
            resumePhase: resumePhaseIfHunter,
            queuedDeaths: deaths.slice(i + 1),
          };
          return;
        }
      }
  ...
  private applyHunterAction(state: WerewolfState, action: Action): ActionResult {
    const consumeActor = (s: WerewolfState) => consumeCurrentActor(s);
    const result = applyHunterAction(state, action, consumeActor);
    if (result.success) {
