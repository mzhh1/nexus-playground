 ...
 * Task Prompts for LLM Executor
    * Contains two versions: with memory and without memory
        */
...
**重要提示**：
1. action_id 必须是完全匹配上述可用行动列表中的某个 action_id的纯字符串，不要包含参数（错误示例：❌ "place(row: 7, col: 6)"）
...
# 任务要求
请分析当前局势，选择一个最优行动。并可选更新记忆。你必须严格按照以下JSON格式返回：
